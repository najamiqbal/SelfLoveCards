package softsolstudio.apps.selflovecards.utils;

public class constants {

    public static String termsAndUse = " <h3><b>About</b>\n" +
            "        </h3>\n" +
            "        <p>\n" +
            "  “Nurture by CStudios is all about you.  This is YOUR time! Your life is full and busy. Sometimes, it seems like there's not enough time in the day to properly nurture or pamper yourself.  This oracle deck provides fun and easy ideas to enhance the wow in your life.  Remember, self-care is not selfish. It's a necessity!” .\n" +
            "            <br>\n" +
            "            <br>\n" +
            "        </p>\n" +
            "\n";

    public static String guide =
            "        <h4 class=\"margin-bottom-0\">\n" +
                    "         Steps workout\n" +
                    "        </h4>\n" +
                    "        <p>\n" +
                    "        * Step 1: Warm up\n" +
                    "* Step 2: Choose a plan you want to workout (28 days\n" +
                    "of challenge or or Abs, Arm, Butt, Leg, Chest from\n" +
                    "Beginner to Advanced)\n" +
                    "* Step 3: Stretching (Optional)" +
                    "            <br>\n" +
                    "            <br>\n" +
                    "        </p>\n" +
                    "        <h4 class=\"margin-bottom-0\">\n" +
                    "     Best time to workout\n" +
                    "        </h4>\n" +
                    "        <p>\n" +
                    "  <img src ='header.png'><br/>" +
                    "        <h4 class=\"margin-bottom-0\">\n" +
                    " Morning workout\n" +
                    "        </h4>\n" +
                    "-Many people find morning workouts to be their\n" +
                    "preferred choice for a variety of reasons. One of the\n" +
                    "most common reasons cited is that when you exercise\n" +
                    "in the morning, you get your workout out of the way.\n" +
                    "Science also backs up morning workouts in some\n" +
                    "regards, as morning exercise tends to increase your\n" +
                    "energy for the rest of the day.\n" +
                    "-In fact, a morning workout is a lot like breakfast in that\n" +
                    "it gets your metabolism going. Simply put, you burn\n" +
                    "more calories all day long just from the sheer fact of\n" +
                    "exercising in the morning. A study conducted at\n" +
                    "Appalachian State University also found that morning\n" +
                    "workouts are preferable if you want a better niqht's\n" +
                    "One more reason to go with morning is that it's been\n" +
                    "shown that people who work out in the morning are\n" +
                    "overall more likely to be consistent with their workouts.\n" +
                    "So pack your breakfast in your meal management bag,\n" +
                    "and dig in post-morning workout session.\n" +
                    "-Morning workouts certainly have their benefits. But\n" +
                    "plenty of people like to exercise at night as well, and\n" +
                    "surely science backs that up to some degree too. The\n" +
                    "real factor to consider is that consistency is key, no\n" +
                    "matter when you choose to work out. But studies have\n" +
                    "shown that different fitness goals are better achieved\n" +
                    "at certain parts of the day, and this is where working\n" +
                    "out later in the day comes into play.\n" +
                    "        <h4 class=\"margin-bottom-0\">\n" +
                    " Afternoon and night workout\n" +
                    "        </h4>\n" +
                    "-Strength and endurance are both higher in the\n" +
                    "afternoon, while the likelihood of injuries is decreased.\n" +
                    "Exercising when body temperature is lowest, which is\n" +
                    "typically later in the day, around 4 or 5 p.m., results in\n" +
                    "improved performance and increased power. At this\n" +
                    "time of day, muscles are more flexible since your body\n" +
                    "is more warmed up than it is in the morning. Your\n" +
                    "reaction time is likely to be quicker, while heart rate\n" +
                    "and blood pressure are low. Protein synthesis peaks at\n" +
                    "this time of day, as well. Based on this, intense\n" +
                    "workouts such as weight training or hard cardio should\n" +
                    "take place during the late afternoon or evening.\n" +
                    "-Not only that, but the calories provided by the small\n" +
                    "meals you have packed in your meal management bag\n" +
                    "are the perfect fuel for a nighttime workout. A study\n" +
                    "conducted at the Clinical Research Center of the\n" +
                    "University of Chicago found that those who hit the gym\n" +
                    "after work are more likely to achieve a higher degree of\n" +
                    "fitness than early bird exercisers. Blood samples\n" +
                    "showed that levels of certain endocrine hormones\n" +
                    "(cortisol and thyrotropin) significantly increased in\n" +
                    "those working out at night. Chalk this up as another\n" +
                    "reason to engage in more strenuous activity at night.\n" +
                    "            <br>\n";


    public static String easyworkout =
            "<h4 class=\"margin-bottom-0\">\n" +
                    "  1. Crunches:\n" +
                    "        </h4>\n" +
                    "  <img src ='crunch_1.jpg'><br/>" +
                    "        <h4 class=\"margin-bottom-0\">\n" +
                    "How To Do?\n" +
                    "        </h4>\n" +
                    "        <p>\n" +
                    "Lie down flat on a mat with your knees bent and\n" +
                    "feet on the round. Alternatively, you can also lift\n" +
                    "your legs o? the floor at a 90-degree angle. (See\n" +
                    "picture).\n" +
                    "Lift your hands and place them behind YOUr head,\n" +
                    "or keep them crossed on your chest.\n" +
                    "Inhale deeply, and as you lift your upper torso off\n" +
                    "the floor, exhale.\n" +
                    "Inhale again as you get back down, and exhale as\n" +
                    "YOU come up.\n" +
                    "Do this for 10 times as a beginner.\n" +
                    "Repeat another two to three sets." +
                    "            <br>\n" +
                    "            <br>\n" +
                    "        </p>\n" +
                    "        <h4 class=\"margin-bottom-0\">\n" +
                    "Precautions\n" +
                    "        </h4>\n" +
                    "        <p>\n" +
                    "While performing crunches, instead of entering\n" +
                    "the full sit-up position, iust raise your back a few\n" +
                    "inches from the ground. This ensures you don't\n" +
                    "hurt your back.\n" +
                    "Also, don't ierk your head forward while doing\n" +
                    "crunches. This will put pressure on your neck and\n" +
                    "result in pain. Just hold your hands above your\n" +
                    "head and perform the exercise.\n" +
                    "            <br>\n" +
                    "            <br>\n" +
                    "        </p>\n" +
                    "<h4 class=\"margin-bottom-0\">\n" +
                    " 2. Side Crunch:\n" +
                    "        </h4>\n" +
                    "  <img src ='side_crunches.jpg'><br/>" +
                    "        <h4 class=\"margin-bottom-0\">\n" +
                    "How To Do?\n" +
                    "        </h4>\n" +
                    "        <p>\n" +
                    " This is same as the twist crunch exercise. The\n" +
                    "only difference is that you need to tilt your legs to\n" +
                    "the same side simultaneously with your\n" +
                    "shoulders. The side crunch focuses on the\n" +
                    "muscles on your sides.\n" +
                    "            <br>\n" +
                    "            <br>\n" +
                    "        </p>\n" +
                    "        <h4 class=\"margin-bottom-0\">\n" +
                    " Precautions\n" +
                    "        </h4>\n" +
                    "        <p>\n" +
                    "Make sure you keep your movements steady and\n" +
                    "slow. The midsection, being a complex area,\n" +
                    "could hurt if you perform the twists in a hurried\n" +
                    "fashion.\n\n" +
                    "            <br>\n" +
                    "            <br>\n" +
                    "        </p>\n" +
                    "<h4 class=\"margin-bottom-0\">\n" +
                    "  3. Reverse Crunches:\n" +
                    "        </h4>\n" +
                    "  <img src ='reverse_crunches.jpg'><br/>" +
                    "        <h4 class=\"margin-bottom-0\">\n" +
                    "How To Do?\n" +
                    "        </h4>\n" +
                    "        <p>\n" +
                    "This is similar to the twist crunch exercise. The\n" +
                    "only thing that you need to do is tilt your legs\n" +
                    "behind simultaneously with your shoulders. The\n" +
                    "side crunch focuses on the muscles on your\n" +
                    "sides.\n" +
                    "            <br>\n" +
                    "            <br>\n" +
                    "        </p>\n" +
                    "        <h4 class=\"margin-bottom-0\">\n" +
                    "    Precautions\n" +
                    "        </h4>\n" +
                    "        <p>\n" +
                    "Keep your back straight while performing the\n" +
                    "exercise, as arching it can result in pain, and in\n" +
                    "some cases, even iniury.\n" +
                    "            <br>\n" +
                    "            <br>\n" +
                    "        </p>\n" +
                    "<h4 class=\"margin-bottom-0\">\n" +
                    "  4. Twist Crunches:\n" +
                    "        </h4>\n" +
                    "  <img src ='twist_crunches.jpg'><br/>" +
                    "        <h4 class=\"margin-bottom-0\">\n" +
                    "How To Do?\n" +
                    "        </h4>\n" +
                    "        <p>\n" +
                    "Lie down on the floor with your hands behind\n" +
                    "your head.\n" +
                    "Bend your knees as you would do in crunches,\n" +
                    "keeping your feet on the floor.\n" +
                    "You have to lift your upper torso while performing\n" +
                    "crunches. But, In twist crunches, you have to lift\n" +
                    "•ust your right shoulder towards the left, keeping\n" +
                    "lhe left side of your torso on the ground.\n" +
                    "Again alternately, lift your left shoulder towards\n" +
                    "the right, keeping the right side of your torso on\n" +
                    "the ground.\n" +
                    "Repeat 10 times.\n" +
                    "            <br>\n" +
                    "            <br>\n" +
                    "        </p>\n" +
                    "        <h4 class=\"margin-bottom-0\">\n" +
                    "    Precautions\n" +
                    "        </h4>\n" +
                    "        <p>\n" +
                    "Keep your back straight while performing the\n" +
                    "exercise, as arching it can result in pain, and in\n" +
                    "some cases, even iniury.\n" +
                    "            <br>\n" +
                    "            <br>\n" +
                    "        </p>\n" +
                    "<h4 class=\"margin-bottom-0\">\n" +
                    "  5. Vertical Leg Crunches:\n" +
                    "        </h4>\n" +
                    "  <img src ='vertical_leg_crunch.png'><br/>" +
                    "        <h4 class=\"margin-bottom-0\">\n" +
                    "How To Do?\n" +
                    "        </h4>\n" +
                    "        <p>\n" +
                    "Lie flat on the floor, or on the mat, with your legs\n" +
                    "extended upwards (towards the ceiling) and one\n" +
                    "knee crossed over the other.\n" +
                    "Now that you have positioned your body perfectly,\n" +
                    "do the same as you would have done in the case\n" +
                    "of crunches. That is, breathe in and lift your\n" +
                    "upper body from the floor towards the pelvis.\n" +
                    "Breathe out slowly. As you bring yourself down,\n" +
                    "breathe in again, and exhale as you go up.\n" +
                    "Do 12 to 15 reps and up to three sets.\n" +
                    "            <br>\n" +
                    "            <br>\n" +
                    "        </p>\n" +
                    "        <h4 class=\"margin-bottom-0\">\n" +
                    "    Precautions\n" +
                    "        </h4>\n" +
                    "        <p>\n" +
                    "Start by doing only a few repetitions of this\n" +
                    "exercise, as IT can make you feel sore if you\n" +
                    "overdo it initially.\n" +
                    "            <br>\n" +
                    "            <br>\n" +
                    "        </p>\n" +
                    "<h4 class=\"margin-bottom-0\">\n" +
                    "  6. Bicycle Exercise:\n" +
                    "        </h4>\n" +
                    "  <img src ='bicycle_crunch.png'><br/>" +
                    "        <h4 class=\"margin-bottom-0\">\n" +
                    "How To Do?\n" +
                    "        </h4>\n" +
                    "        <p>\n" +
                    "Lie on the floor and keep your hands either by\n" +
                    "your sides or behind your head as YOU do in\n" +
                    "crunches.\n" +
                    "Lift both your legs off the ground and bend them\n" +
                    "at the knees.\n" +
                    "Bring your right knee close to YOUr chest, keeping\n" +
                    "your left leg away.\n" +
                    "Now take your right leg away and bring your left\n" +
                    "leg close to your chest.\n" +
                    "Keep doing this as if you are paddling a bicycle.\n" +
                    "            <br>\n" +
                    "            <br>\n" +
                    "        </p>\n" +
                    "<h4 class=\"margin-bottom-0\">\n" +
                    "  7. Lunge Twist:\n" +
                    "        </h4>\n" +
                    "  <img src ='lunge_twist.jpg'><br/>" +
                    "        <h4 class=\"margin-bottom-0\">\n" +
                    "How To Do?\n" +
                    "        </h4>\n" +
                    "        <p>\n" +
                    "Stand with your legs hip width apart. Keep YOUr\n" +
                    "knees slightly bent.\n" +
                    "Lift both your hands in front of YOU, aligning them\n" +
                    "with your shoulders and keeping them parallel to\n" +
                    "the ground.\n" +
                    "Lunge forward as shown in the picture. Take a big\n" +
                    "step forward with your right leg, and sit down as IT\n" +
                    "on a chair so that your knees make a 90-degree\n" +
                    "angle with the floor. The left leg should be\n" +
                    "positioned backwards, supported by the toes.\n" +
                    "The spine should be kept straight. Don't bend\n" +
                    "your spine forward.\n" +
                    "Twist your torso (iust the torso, and not the legs)\n" +
                    "to the right and then to the left.\n" +
                    "Repeat 15 times.\n" +
                    "            <br>\n" +
                    "            <br>\n" +
                    "        </p>\n" +
                    "<h4 class=\"margin-bottom-0\">\n" +
                    "  8. Stomach Vacuum:\n" +
                    "        </h4>\n" +
                    "  <img src ='header.png'><br/>" +
                    "        <h4 class=\"margin-bottom-0\">\n" +
                    "How To Do?\n" +
                    "        </h4>\n" +
                    "        <p>\n" +
                    "This is similar to what we call the cat stretch\n" +
                    "pose. This is also known as the four-point\n" +
                    "transverse-abdominal stomach vacuum. follow\n" +
                    "the steps mentioned below to do this exercise for\n" +
                    "reducing belly fat:\n" +
                    "Go down to the ground on all fours, supporting\n" +
                    "your body on your hands and knees.\n" +
                    "Inhale deeply and loosen your abdomen.\n" +
                    "As you exhalez tighten the abdomen muscles.\n" +
                    "Hold this position for 15-30 seconds.\n" +
                    "Repeat the process.\n\n" +
                    "            <br>\n" +
                    "            <br>\n" +
                    "        </p>\n" +
                    "<h4 class=\"margin-bottom-0\">\n" +
                    "  9. Rolling PLank:\n" +
                    "        </h4>\n" +
                    "  <img src ='rolling_plank.jpg'><br/>" +
                    "        <h4 class=\"margin-bottom-0\">\n" +
                    "How To Do?\n" +
                    "        </h4>\n" +
                    "        <p>\n" +
                    "Position yourself on the floor with your knees and\n" +
                    "elbows resting on the ground.\n" +
                    "Keep your neck aligned with your spine. Look\n" +
                    "forward.\n" +
                    "Lift the knees UP and support your legs on the\n" +
                    "toes.\n" +
                    "Contract YOUr knees and keep breathing\n" +
                    "normally.\n" +
                    "This is the plank pose. Stay in this posture for 30\n" +
                    "seconds.\n" +
                    "Now, start moving to and fro for the next 30\n" +
                    "seconds. This is the rolling plank exercise.\n" +
                    "Lie down on the floor sideways.\n" +
                    "Support yourself on your right elbow and right\n" +
                    "leg. Your elbow should be perpendicular to the\n" +
                    "shoulder, and the left leg should be above the\n" +
                    "right leg, keeping them together.\n" +
                    "Keep your knees straight. Your hips should not be\n" +
                    "touching the ground.\n" +
                    "Hold this posiTion for 30 seconds. Once you get\n" +
                    "comfortable, you may hold this for one to two\n" +
                    "minutes.\n" +
                    "Repeat on the other side too.\n" +
                    "While you are in this position you may also lift\n" +
                    "the leg on the top and bring if down again. This\n" +
                    "makes the exercise more effective - it works not\n" +
                    "only your abs, but also your thighs and hips.\n" +
                    "            <br>\n" +
                    "            <br>\n" +
                    "        </p>\n" +
                    "        <h4 class=\"margin-bottom-0\">\n" +
                    "    Precautions\n" +
                    "        </h4>\n" +
                    "        <p>\n" +
                    "Plank is a strenuous exercise, and YOU might feel\n" +
                    "like holding your breath while performing it. But\n" +
                    "don't do that, as you might suffer from nausea or\n" +
                    "dizziness.\n" +
                    "            <br>\n" +
                    "            <br>\n" +
                    "        </p>\n";

    public static String drinks14days = " <h4><b>Day 1</b>\n" +
            "        </h4>\n" +
            "        <p>\n" +
            "           Mix together one squeezed lemon with a cup of water and consume right after waking UP.\n" +
            "            <br>\n" +
            "        </p>\n" +
            "        <h4 class=\"margin-bottom-0\">\n" +
            "         Day 2\n" +
            "        </h4>\n" +
            "        <p>\n" +
            "       Mix together two lemons into two CUPS of water. You may add honey for taste. Consume it before eating breakfast.\n" +
            "            <br>\n" +
            "            <br>\n" +
            "        </p>\n" +
            "        <h4 class=\"margin-bottom-0\">\n" +
            "         Day 3\n" +
            "        </h4>\n" +
            "        <p>\n" +
            "      Mix together the juice from three lemons with three CUPS of water and a little honey." +
            "            <br>\n" +
            "            <br>\n" +
            "        </p>\n" +
            "        <h4 class=\"margin-bottom-0\">\n" +
            "         Day 4\n" +
            "        </h4>\n" +
            "        <p>\n" +
            "     Mix together the juice from four lemons with four CUPS of water and a little honey. Consume it twice a day, in the morning and in the evening before going to bed.  \n" +
            "            <br>\n" +
            "            <br>\n" +
            "        </p>\n" + "     " +
            "   <h4 class=\"margin-bottom-0\">\n" +
            "         Day 5\n" +
            "        </h4>\n" +
            "        <p>\n" +
            "      Squeeze five lemons and mix the juice with 5 CUPS of water and honey. Drink it twice a day. \n" +
            "            <br>\n" +
            "            <br>\n" +
            "        </p>\n" + "" +
            "        <h4 class=\"margin-bottom-0\">\n" +
            "         Day 6\n" +
            "        </h4>\n" +
            "        <p>\n" +
            "    Mix together the juice from six lemons with six CUPS of water and a little honey. Consume the mixture three times a day.   \n" +
            "            <br>\n" +
            "            <br>\n" +
            "        </p>\n" + "    " +
            "    <h4 class=\"margin-bottom-0\">\n" +
            "         Day 7\n" +
            "        </h4>\n" +
            "        <p>\n" +
            "       Mix together the juice from three lemons and 10 CUPS of water. Consume it three times a day before meals.\n" +
            "            <br>\n" +
            "            <br>\n" +
            "        </p>\n" + " " +
            "       <h4 class=\"margin-bottom-0\">\n" +
            "         Day 8\n" +
            "        </h4>\n" +
            "        <p>\n" +
            "      Use 6 lemons and 6 CUPS of water to make the drink. \n" +
            "            <br>\n" +
            "            <br>\n" +
            "        </p>\n" + "" +
            "        <h4 class=\"margin-bottom-0\">\n" +
            "         Day 9\n" +
            "        </h4>\n" +
            "        <p>\n" +
            "      Drink juice from 5 lemons, 5 CUPS of water and a tablespoon of honey. \n" +
            "            <br>\n" +
            "            <br>\n" +
            "        </p>\n" + "      " +
            "  <h4 class=\"margin-bottom-0\">\n" +
            "         Day 10\n" +
            "        </h4>\n" +
            "        <p>\n" +
            "      Mix 4 lemons with 4 CUPS of water and one tablespoon of honey. \n" +
            "            <br>\n" +
            "            <br>\n" +
            "        </p>\n" + "   " +
            "    <h4 class=\"margin-bottom-0\">\n" +
            "         Day 11\n" +
            "        </h4>\n" +
            "        <p>\n" +
            "       Squeeze 3 lemons and mix the juice with 3 CUPS of water. Add a teaspoon of honey and drink it twice a day.\n" +
            "            <br>\n" +
            "            <br>\n" +
            "        </p>\n" +
            "        <h4 class=\"margin-bottom-0\">\n" +
            "         Day 12\n" +
            "        </h4>\n" +
            "        <p>\n" +
            "      Mix lemon juice from 2 lemons with 2 CUPS of water and drink it In the morning before breakfast. \n" +
            "            <br>\n" +
            "            <br>\n" +
            "        </p>\n" +
            "        <h4 class=\"margin-bottom-0\">\n" +
            "         Day 13\n" +
            "        </h4>\n" +
            "        <p>\n" +
            "      Drink juice from one lemon and one cup of water. \n" +
            "            <br>\n" +
            "            <br>\n" +
            "        </p>\n" +
            "        <h4 class=\"margin-bottom-0\">\n" +
            "         Day 14\n" +
            "        </h4>\n" +
            "        <p>\n" +
            "     Prepare a drink from 3 lemons and 10 CUPS of water. Also, add a tablespoon of honey before you consume it.  \n" +
            "            <br>\n" +
            "            <br>\n" +
            "        </p>\n";
}
